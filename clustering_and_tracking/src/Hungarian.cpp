...
double Solve(vector<vector<double>> DistMatrix, vector<int>& Assignment) {
    int rows = DistMatrix.size();
    if (rows == 0) {
        return 0.0;
    }
    int cols = DistMatrix[0].size();

    vector<tuple<double, int, int>> elements;
    for (int i = 0; i < rows; ++i) {
        for (int j = 0; j < cols; ++j) {
            elements.emplace_back(DistMatrix[i][j], i, j);
        }
    }
    ...
    vector<bool> row_used(rows, false);
    vector<bool> col_used(cols, false);
    ...
    for (int i = 0; i < rows; ++i) {
        if (Assignment[i] != -1) {
            total_cost += DistMatrix[i][Assignment[i]];
        }
    }
