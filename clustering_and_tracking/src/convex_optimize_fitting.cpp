...
void ConvexOptimizeFitting::calcuBoxbyPolygon(double &theta_star, Eigen::Vector3d &position, Eigen::Vector3d &size, double &yaw,
                                              const std::vector<cv::Point2f> &polygon_cluster, const pcl::PointCloud<pcl::PointXYZI> &cluster)
{
  ...
  // set output
  position << (intersection_x_1 + intersection_x_2) / 2.0, (intersection_y_1 + intersection_y_2) / 2.0, (max_z + min_z) / 2.0;

  size << std::fabs(e_x.dot(diagonal_vec)), std::fabs(e_y.dot(diagonal_vec)), std::max((max_z - min_z), 0.01);
}
...
void ConvexOptimizeFitting::bbox_fitting(pcl::PointCloud<pcl::PointXYZI> current_cluster, Eigen::Vector3d &position, Eigen::Vector3d &size, double &yaw)
{
    ...
    calcuBoxbyPolygon(theta_optim, position,size, yaw, ch_cluster, current_cluster);
}
