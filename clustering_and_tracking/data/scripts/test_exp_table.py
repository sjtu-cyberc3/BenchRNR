...
def plot_error_comparison(df):
    ...
    methods = ['Seg+obb', 'Seg+convex', 'Register-loc', 'PV-RCNN']
    str_nums = ['m', 'n', 'm_64']
    ...
    bar_width = 0.25  # Adjusted the bar width to fit three bars per group

    # Create a figure with two subplots
    fig, ax1 = plt.subplots(1, 1, figsize=(6, 4))  # 1 row, 2 columns
    ...
    for i in range(len(methods)):
        ax1.text(index[i] - bar_width, rep_loc[i, 0], f'{rep_loc[i, 0]:.1f}', ha='center', va='bottom', fontsize=10)
        ax1.text(index[i], rep_loc[i, 1], f'{rep_loc[i, 1]:.1f}', ha='center', va='bottom', fontsize=10)
        ax1.text(index[i] + bar_width, rep_loc[i, 2], f'{rep_loc[i, 2]:.1f}', ha='center', va='bottom', fontsize=10)
